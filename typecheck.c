...
static void check_func(T_func func)
{
  ...
  while( paramlist != NULL && paramtypes != NULL )
  {
  ...
  if( paramlist != NULL || paramtypes != NULL )
  {
...
static void check_ifstmt(T_stmt stmt) {
  ...
  {
       type_error( "if condition myst be an int" );
  }
...
static void check_whilestmt(T_stmt stmt)
{
    ...
    {
         type_error("while statment condition must be of type INT\n");
    }
...
static void check_callexpr(T_expr expr)
{
    ...
    if ( NULL == binding )
    {
         type_error("function call to undefined symbol\n");
    }
    ...
    {
         type_error("the number of arguments of the function call does not match function type");
    }
...
static void check_castexpr( T_expr expr )
{
    ...
    switch( expr->castexpr.expr->type->kind )
    {
      ...
      case E_pointertype:
        ...
        break;
      case E_functiontype:
        type_error( "cannot type cast function type" );
      default:
